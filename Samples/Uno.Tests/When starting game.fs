...
let [<Fact>] ``Started game should be started``() =
    test <@ replay []
        |> handle <| StartGame { GameId = gameId; PlayerCount = 4; FirstCard = red 3 } 
        =[ GameStarted { GameId = gameId; PlayerCount = 4; FirstCard = red 3; FirstPlayer = 0 } ] @> 

let [<Fact>] ``0 players should be rejected``() =
    <@ replay []
    |> handle <| StartGame { GameId = gameId; PlayerCount = 0; FirstCard = red 3 } @>
    |> raises<ArgumentException>

let [<Fact>] ``Game should not be started twice``() =
    <@ replay [GameStarted { GameId = gameId; PlayerCount = 4; FirstCard = red 3; FirstPlayer = 0 } ]
    |> handle <| StartGame { GameId = gameId; PlayerCount = 4; FirstCard = red 2 }  @>
    |> raises<InvalidOperationException>

let [<Fact>] ``Starting with a kickback should change direction``() =
    test <@ replay []
        |> handle <| StartGame { GameId = gameId; PlayerCount = 4; FirstCard = KickBack Yellow }
...
let [<Fact>] ``Starting with a skip should skip dealer's turn``() =
    test <@ replay []
        |> handle <| StartGame { GameId = gameId; PlayerCount = 4; FirstCard = Skip Yellow }
