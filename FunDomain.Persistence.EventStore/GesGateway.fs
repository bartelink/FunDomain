...
module Async =
    let AwaitTaskVoid : (Threading.Tasks.Task -> Async<unit>) =
...
module private EventStoreExtensions =
    type EventStore.ClientAPI.IEventStoreConnection with
        member this.AsyncConnect () = 
            Async.AwaitTaskVoid <| this.ConnectAsync() 
        ...
        member this.AsyncSubscribeToStream stream resolveLinkTos onEvent onDrop =
            Async.AwaitTask <| this.SubscribeToStreamAsync(stream, resolveLinkTos, Action<_,_>(onEvent), Action<_,_,_>(onDrop), null) 
...
type Store private (inner') = 
    ...
    let toEncodedEvent (e:ResolvedEvent) = { EventType = e.Event.EventType; Data = e.Event.Data }
    ...
    let appendWithIdGenerator generator streamId expectedVersion newEncodedEvents = 
        async {
            ...
            let! wr =
                [| for e in newEncodedEvents -> EventData(generator e.Data, e.EventType, isJson, e.Data, metadata) |]
    ...
    member this.subscribeAll (username,password) projection =
        inner.AsyncSubscribeToAll (*resolveLinkTos*)true (dispatchWithStreamIdTo projection) (SystemData.UserCredentials(username,password))

    member this.subscribeStream stream projection =
        inner.AsyncSubscribeToStream stream false (dispatchTo projection) (fun _ _ _ -> ())
